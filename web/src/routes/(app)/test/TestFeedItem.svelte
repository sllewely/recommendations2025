<script lang="ts">
    ...
    let by_line = function() {
        switch(feed_item.class_name) {
            ...
            case 'Event': return ' posted an upcoming event';
            default: return " is interested in";
        }
    }();
...
<div class="p-2">
    ...
    <Card>
        ...
        <div class="flex flex-row justify-between pb-2">
            <div><span class="font-bold"><Link url="/users/{feed_item.creator_id}">{feed_item.creator_name}</Link></span></div>
            <div><span class="text-sm">at {feed_item.create_date_string} {feed_item.create_time_string}</span> {by_line}</div>
        </div>
